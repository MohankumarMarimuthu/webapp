...
const EditListing = () => {
  ...
  useEffect(() => {
     const fetchApiListing = async() => {
        ...
        const res = await fetch(`/api/list/get-listing-byId/${listingId}`)
  ...
  const handleSubmit = async(e : any)  => {
    ...
    try{
      ...
      const res = await fetch(`/api/list/edit/${listingId}` , {
        method : 'POST',
        headers : {
  ...
  return (
    <main className='p-3 max-w-4xl mx-auto'>
      <h1 className='font-semibold my-7 text-center text-3xl'>Edit listing</h1>
      <form className='flex flex-col md:flex-row gap-6' onSubmit={handleSubmit}>
        ...
        <div className='flex flex-col flex-1 gap-4'>
          ...
          disabled:opacity-80' disabled={loading}>{loading ? "updating" : "update listing"}</button>
